...
Chess::GenericMove OpeningBook::moveFromBits(quint16 pgMove)
{
	using Chess::Square;
...
quint16 OpeningBook::moveToBits(const Chess::GenericMove& move)
{
	using Chess::Square;
...
Chess::GenericMove OpeningBook::Entry::move() const
{
	return OpeningBook::moveFromBits(pgMove);
}
...
int OpeningBook::import(const PgnGame& pgn, int maxMoves)
{
	...
	for (int i = 0; i < maxMoves; i++)
	{
		...
		if ((i % 2) != loserMod)
		{
			auto pg_move = moveToBits(moves.at(i).move);
			Entry entry = { pg_move, weight };
...
Chess::GenericMove OpeningBook::move(quint64 key) const
{
	...
	for (const Entry& entry : entries)
	{
		...
		if (currentWeight > pick)
			return moveFromBits(entry.pgMove);
	}
