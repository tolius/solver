...
bool PgnGame::write(QTextStream& out, PgnMode mode) const
{
	...
	if (mode == Minimal && m_tags.contains("Variant") && variant() != "standard")
	{
		writeTag(out, "Variant", m_tags["Variant"]);
	}

	int lineLength = writeMoves(out, false, mode);

	QString str = m_tags.value("Result");
...
int PgnGame::writeMoves(QTextStream& out, bool addVariantTag, PgnMode mode) const
{
	if (addVariantTag && variant() != "standard")
	{
