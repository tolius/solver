...
class LIB_EXPORT Board
{
	...
	public:
	    /*! Returns all the moves made in the game. */
	    const QVector<MoveData>& MoveHistory() const;
...
inline const QVector<Board::MoveData>& Board::MoveHistory() const
{
...
inline size_t Board::numPieces() const
{
	size_t num = 0;
	for (const auto& s : m_squares)
		if (s.isValid())
			num++;
	return num;
}
