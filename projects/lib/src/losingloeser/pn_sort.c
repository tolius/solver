...
void ratio_sort_children(PN_NODE* tree, uint32 node)
{
	...
	n = S[0].n;
	tree[node].child = S[0].n;
	for (i = 1; i < u; i++)
	{
		tree[n].sibling = S[i].n;
		n = S[i].n;
	}
	tree[n].sibling = 0;
}
...
void final_sort_children(PN_NODE* tree, uint32 node)
{
	...
	int u = 0;
	uint32 n = tree[node].child;
	S[u].size = tree[n].size;
	S[u].bad = tree[n].bad;
	S[u].good = tree[n].good;
	S[u++].n = n;
	while (tree[n].sibling)
	{
		n = tree[n].sibling;
