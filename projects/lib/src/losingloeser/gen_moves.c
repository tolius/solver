...
boolean MakeMoveCheck(typePOS* POSITION, uint16 mv, boolean CHECK)
{
	...
	if (mv & (1 << 15)) /* ep */
	{
		...
		if (POSITION->wtm)
		{
			bBitboardOcc ^= SqSet[z];
			bBitboardP ^= SqSet[z];
			POSITION->DYN->HASH ^= Zobrist(bEnumP, z);
		}
		else
		{
			wBitboardOcc ^= SqSet[z];
			wBitboardP ^= SqSet[z];
	...
	for (i = 4; i <= POSITION->DYN->rev; i += 2)
		...
		{
			UnmakeMove(POS, POS->DYN->mv, TRUE);
			return FALSE;
		}
	return TRUE;
}
...
void UnmakeNullMove(typePOS* POSITION)
{
	POSITION->wtm ^= 1;
	POSITION->DYN--;
}
...
static int GenMovesWhite(typePOS* POS, uint16* ml)
{
	...
	*ml = 0;
	...
	for (j = 2; j <= 6; j++)
		for (U = POSITION->bitboard[j]; U; U &= (U - 1))
		{
			sq = BSF(U);
			T = Attacks(POS, j, sq) & bBitboardOcc;
			while (T)
			{
				to = BSF(T);
				T &= (T - 1);
	...
	for (j = 2; j <= 6; j++)
		...
		{
			sq = BSF(U);
			T = Attacks(POS, j, sq) & ~POS->OCCUPIED;
			while (T)
			{
				to = BSF(T);
				T &= (T - 1);
...
static int GenMovesBlack(typePOS* POS, uint16* ml)
{
	...
	*ml = 0;
	...
	for (j = 2; j <= 6; j++)
		for (U = POSITION->bitboard[7 + j]; U; U &= (U - 1))
		{
			sq = BSF(U);
			T = Attacks(POS, j, sq) & wBitboardOcc;
			while (T)
			{
				to = BSF(T);
				T &= (T - 1);
...
void Validate(typePOS* POS)
{
	...
	if (POSITION->wtm)
		Z ^= ZobristWTM;
	if (POSITION->DYN->ep)
		Z ^= ZobristEP[POSITION->DYN->ep & 7];
	if (0 && Z != POS->DYN->HASH) // if 0
	{
		BAD = TRUE;
		printf("Z:%llx HASH:%llx\n", Z, POS->DYN->HASH);
	}
	if (POS->OCCUPIED != (wBitboardOcc | bBitboardOcc))
	{
		BAD = TRUE;
		printf("O:%llx w:%llx b:%llx\n", POS->OCCUPIED, wBitboardOcc, bBitboardOcc);
	}
	O = 0;
	for (pi = 1; pi <= 6; pi++)
		O |= POSITION->bitboard[pi];
	if (O != wBitboardOcc)
	{
		BAD = TRUE;
		printf("Ow:%llx %llx\n", O, wBitboardOcc);
